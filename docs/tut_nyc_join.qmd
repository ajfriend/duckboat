...
To perform a join, we need two tables in hand, which we can do with `darkwing.Database()`:
...
You can run DuckDB SQL on a `darkwing.Database`, but now you should explicitly mention the table(s) you want to work with (but that's usually what you want anyway when doing a join.)
