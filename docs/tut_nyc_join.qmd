...
You can execute one or more SQL expressions with the `Table.do()` method.
...
To perform a join, pass a dict to `do()` to register named tables:
...
t1 = uck.do('data/yellow_tripdata_2010-01.parquet', core_with_filters)
t2 = uck.do('data/yellow_tripdata_2010-02.parquet', core_with_filters)
